...
Supports both Parquet (.parquet) and LAZ (.laz, .las) files.
Allows selecting up to three files for comparison.
"""
...
# Color scheme for three files
COLOR_FILE_1 = [1.0, 0.3, 0.3]      # Red
COLOR_FILE_2 = [0.2, 0.5, 1.0]      # Blue
COLOR_FILE_3 = [0.3, 0.8, 0.3]      # Green
...
def load_parquet_points(path: Path) -> Optional[np.ndarray]:
    ...
    try:
        ...
        print(f"Loaded {len(points):,} points from parquet")
        return points
...
def load_laz_points(path: Path) -> Optional[np.ndarray]:
    ...
    try:
        ...
        print(f"Loaded {len(points):,} points from LAZ/LAS")
        return points

    except Exception as e:
        print(f"Error loading LAZ/LAS {path}: {e}")
        return None


def load_points(path: Path) -> Optional[np.ndarray]:
    """Load point cloud from file (auto-detects format).
    ...
    if suffix == '.parquet':
        return load_parquet_points(path)
    elif suffix in ['.laz', '.las']:
        return load_laz_points(path)
    else:
...
class ViewerGUI:
    ...
    def create_widgets(self):
        ...
        # File 3 section
        file3_frame = ttk.LabelFrame(main_frame, text="File 3 (Green)", padding="10")
        file3_frame.grid(row=3, column=0, columnspan=3, padx=5, pady=5, sticky=(tk.W, tk.E))

        ttk.Checkbutton(file3_frame, text="Show File 3", variable=self.show_file3).grid(row=0, column=0, sticky=tk.W, padx=5, pady=5)
        ttk.Entry(file3_frame, textvariable=self.file3_path, width=60).grid(row=1, column=0, padx=5, pady=5)
        ttk.Button(file3_frame, text="Browse", command=self.browse_file3).grid(row=1, column=1, padx=5, pady=5)

        # Legend
        legend_frame = ttk.LabelFrame(main_frame, text="Color Legend", padding="10")
        legend_frame.grid(row=4, column=0, columnspan=3, padx=5, pady=5, sticky=(tk.W, tk.E))

        legend_text = "Red: File 1  |  Blue: File 2  |  Green: File 3"
    ...
    def browse_file1(self):
        ...
        path = filedialog.askopenfilename(
            ...
            filetypes=[
                ("Point Cloud Files", "*.parquet *.laz *.las"),
                ("Parquet Files", "*.parquet"),
                ("LAZ/LAS Files", "*.laz *.las"),
                ("All Files", "*.*")
            ],
            initialdir=Path.cwd(),
        )
    ...
    def browse_file3(self):
        """Browse for File 3."""
        path = filedialog.askopenfilename(
            title="Select Point Cloud File 3",
            filetypes=[
                ("Point Cloud Files", "*.parquet *.laz *.las"),
                ("Parquet Files", "*.parquet"),
                ("LAZ/LAS Files", "*.laz *.las"),
    ...
    def view_clouds(self):
        ...
        # Load File 3
        if self.show_file3.get():
            file3_str = self.file3_path.get().strip()
            if file3_str:
                file3_path = Path(file3_str)
                if file3_path.is_file():
                    points = load_points(file3_path)
                    if points is not None and points.size > 0:
                        pc = create_point_cloud(points, COLOR_FILE_3)
                        geometries.append(pc)
                        print(f"Loaded File 3: {file3_path.name} ({len(points):,} points)")
                    else:
                        print(f"Warning: Could not load File 3: {file3_path}")
                else:
                    messagebox.showwarning("File Not Found", f"File 3 does not exist:\n{file3_path}")
